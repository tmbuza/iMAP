...
### Renaming the categoric variables with meaningful names
...
names(variables2) <- c("SampleID", "Sex", "Time", "DayID", "DPW")

# New names
variables2 %>% head(5) %>% kable()

```

## Status of example metadata
...
sexhistplot <- ggpubr::gghistogram(variables2, x = "Sex", stat="count", fill = "#ff0000") + ggtitle("Sex variable")

timehistplot <- ggpubr::gghistogram(variables2, x = "Time", stat="count", fill = "#00ff00") + ggtitle("Time variable")

# scaleFUN <- function(x) sprintf("%.0f", x)

dpwhistplot <- ggpubr::gghistogram(variables2, x = "DayID", stat="count", fill = "#0000ff") + ggtitle("Days post weaning")+
  theme(axis.text.x=element_text(angle = 90, hjust = 0))+ scale_y_continuous(labels=scaleFUN)
