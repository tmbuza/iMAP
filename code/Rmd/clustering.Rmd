...
ggplot(melt(p.scaled.t), aes(x=Var1, y=Var2, fill=value)) + 
  ...
  labs(x="Samples (n = 360)", y="") + theme(axis.text.y = element_text(face = "italic", size = 10)) + noxlabels
...
```{r fviz_silhouette, fig.height=7, fig.width=10, message=FALSE, warning=FALSE}
...
# Visualize pam
otu.fviz.t <- factoextra::fviz_silhouette(otu.pam.t, diss=FALSE, 2) + ggtitle("OTUs")+ normalaxislayout2 + noxlabels + xlab("Data point") + ylab("Si width")
p.fviz.t <- factoextra::fviz_silhouette(p.pam.t, diss=FALSE, 3) + ggtitle("Phylum")+ normalaxislayout2 + noxlabels + xlab("Data point") + ylab("Si width")
c.fviz.t <- factoextra::fviz_silhouette(c.pam.t, diss=FALSE, 3) + ggtitle("Class")+ normalaxislayout2 + noxlabels + xlab("Data point") + ylab("Si width")
o.fviz.t <- factoextra::fviz_silhouette(o.pam.t, diss=FALSE, 2) + ggtitle("Order")+ normalaxislayout2 + noxlabels + xlab("Data point") + ylab("Si width") 
f.fviz.t <- factoextra::fviz_silhouette(f.pam.t, diss=FALSE, 2) + ggtitle("Family")+ normalaxislayout2 + noxlabels + xlab("Data point") + ylab("Si width") 
g.fviz.t <- factoextra::fviz_silhouette(g.pam.t, diss=FALSE, 2) + ggtitle("Genus")+ normalaxislayout2 + noxlabels + xlab("Data point") + ylab("Si width") 
...
```{r sampleclusters, fig.height=7, fig.width=10}
...
## Subsetting for data-driven hypotheses

<hr>

Example: Early vs Late DPW

* Early = First 10 days of treatment
* Late = Last 10 days of treatment
* Samples: Female number 3 and Male number 3
```{r eval=FALSE, include=FALSE}

cat("\nIndex numbers for Female early & late DPW\n")
tenD3early

cat("\nIndex numbers for Male early & late DPW\n")
tenD3late

otushared <- read_table2("./data/mothur/phylotype/qced.trim.contigs.good.unique.good.filter.unique.precluster.pick.seed.wang.pick.pick.tx.shared")

otusubset.shared <- otushared[c(tenD3early, tenD3late),]

write.table(otusubset.shared,"results/tables/otusubset.shared", append = FALSE, quote = FALSE, sep="\t", row.names = FALSE, col.names = TRUE, qmethod = c("escape", "double"), fileEncoding = "")
...
```{r fviz_silhouette_intext, fig.width=10, message=FALSE, warning=FALSE,fig.height=7}
# <!-- ########################## -->
# <!-- ########################## -->
# <!-- ########################## -->
# <!-- ########################## -->
...
# otu.pam.t <- pam(otu.t[c(tenD3early,tenD3late),], 2, metric = "euclidean", stand = FALSE)
# p.pam.t <- pam(phylum.t[c(tenD3early,tenD3late),], 2, metric = "euclidean", stand = FALSE)
# c.pam.t <- pam(class.t[c(tenD3early,tenD3late),], 2, metric = "euclidean", stand = FALSE)
# o.pam.t <- pam(order.t[c(tenD3early,tenD3late),], 2, metric = "euclidean", stand = FALSE)
# f.pam.t <- pam(family.t[c(tenD3early,tenD3late),], 2, metric = "euclidean", stand = FALSE)
# g.pam.t <- pam(genus.t[c(tenD3early,tenD3late),], 2, metric = "euclidean", stand = FALSE)
...
```{r fviz_screeplot, eval=FALSE, fig.height=8, fig.width=10, include=FALSE}
...
```{r stressplot, fig.height=10, fig.width=10}
par(mfrow=c(4,3))
