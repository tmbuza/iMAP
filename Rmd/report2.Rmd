...
```{r loadreaddata, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
library(dplyr)
...
f_qc0read.v <- cbind(murinevars, qc0statR1[,6:8])
r_qc0read.v <- cbind(murinevars, qc0statR2[,6:8])

f_qc0read.v.m <- reshape2::melt(f_qc0read.v, id = c("SampleID","origGroup","Sex","Time","DayID","DPW"))
r_qc0read.v.m <- reshape2::melt(r_qc0read.v, id = c("SampleID","origGroup","Sex","Time","DayID","DPW"))
...
readcountR1.v <- cbind(murinevars[,-6], qcstatsR1[,-1])
readcountR2.v <- cbind(murinevars[,-6], qcstatsR2[,-1])
...
f_readcount.v.m <- reshape2::melt(readcount.v[,-c(9:11)], id = c("SampleID", "origGroup" , "Sex","Time","DayID"))
r_readcount.v.m <- reshape2::melt(readcount.v[,-c(6:8)], id = c("SampleID", "origGroup" , "Sex","Time","DayID"))
